...
export const createFetchTransactionRepository = (): TransactionRepository => {
    return {
        createTransaction: async (transaction: TransactionEntity): Promise<number> => {
            const request = await axiosService.post<ResponseRequest<{IdTransaction: number}>>('/transaction', transaction);
            return request.data.data.IdTransaction;
        },
        getTransaction: async (transactionId: number): Promise<TransactionResponse> => {
            const request = await axiosService.get<ResponseRequest<TransactionResponse>>(`/transaction/${transactionId}`);
